package com.forumias.beta.ui.deta.forumias.home.home_announcement;
...
public class MyLatestAnnouncementPostAdapter extends RecyclerView.Adapter<MyLatestAnnouncementPostAdapter.ViewHolder> {
    private List<HomeLatestPostModel.MyLatestPost> mySpaceList;
    private Context context;
    private BookMarkInterface bookMarkInterface;
    private int loginUserId;

    public MyLatestAnnouncementPostAdapter(List<HomeLatestPostModel.MyLatestPost> mySpaceList, Context context) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        if (mySpaceList.get(position).getType() == 0) {
            ...
            if(mySpaceList.get(position).getIsDiscussion() == BaseUrl.IS_DISCUSSION){
        ...
        if(mySpaceList.get(position).getGroupInfo().size() != 0){

            for(int i = 0; i < mySpaceList.get(position).getGroupInfo().size(); i++){
                if(mySpaceList.get(position).getGroupInfo().get(i).getIs_page() == BaseUrl.SHOW_PAGE_INFO) {
                    holder.tvName.setVisibility(View.VISIBLE);
                    holder.tvName.setText(mySpaceList.get(position).getGroupInfo().get(i).getTitle());
                    Glide.with(context).load(BaseUrl.PAGE_IMAGE_URL+mySpaceList.get(position).getGroupInfo().get(i).getTag_img()).into(holder.ivUserImage);
                    holder.tvUserVerifiedInfo.setVisibility(View.GONE);
                    new Utility().showUserVerified(mySpaceList.get(position).getGroupInfo().get(i).getIs_verified(), holder.tvUserVerified);
                    Log.e("data==page==> " , "user=====> "+mySpaceList.get(position).getGroupInfo().get(i).getTitle());

                    String mySignature = " Created Post " + new DateFormatUtils().showPostDate(mySpaceList.get(position).getCreatedAt());
                    holder.tvPostInfo.setText(mySignature);
                }else{//========
                    Log.e("data==user==> " , "user=====> ");
                    if (mySpaceList.get(position).getUserInfo() != null) {
                        if (mySpaceList.get(position).getUserInfo().getHideRealName() != BaseUrl.HIDE_REAL_NAME) {
                            holder.tvName.setVisibility(View.VISIBLE);
                            holder.tvName.setText(mySpaceList.get(position).getUserInfo().getName());
                            new Utility().showUserVerified(mySpaceList.get(position).getUserInfo().getIsVerified(), holder.tvUserVerifiedInfo);
                        }

                        Glide.with(context).load(mySpaceList.get(position).getUserInfo().getImage()).into(holder.ivUserImage);
                        if (mySpaceList.get(position).getUserInfo().getAbout() != null) {
                            String mySignature =
                                    " (" + mySpaceList.get(position).getUserInfo().getAbout() + ") " +
                                            " Created Post " + new DateFormatUtils().showPostDate(mySpaceList.get(position).getCreatedAt());
                            holder.tvPostInfo.setText(mySignature);
                        } else {
                            String mySignature = " Created Post " + new DateFormatUtils().showPostDate(mySpaceList.get(position).getCreatedAt());
                            holder.tvPostInfo.setText(mySignature);
                        }
                        holder.tvPostUserInfo.setText( "@" + mySpaceList.get(position).getUserInfo().getName());
                        new Utility().showUserVerified(mySpaceList.get(position).getUserInfo().getIsVerified() , holder.tvUserVerifiedInfo);
            ...
            if (mySpaceList.get(position).getUserInfo() != null) {
                ...
                holder.tvPostUserInfo.setVisibility(View.VISIBLE);
                holder.tvPostUserInfo.setText( "@" + mySpaceList.get(position).getUserInfo().getName());
                new Utility().showUserVerified(mySpaceList.get(position).getUserInfo().getIsVerified() , holder.tvUserVerifiedInfo);
    ...
    /* if(mySpaceList.get(position).getTagId() == 0 && mySpaceList.get(position).getChannelId() == 0){
            ...
            if(mySpaceList.get(position).getTagId() != 0){
               ...
               *//* if(mySpaceList.get(position).getChannelInfo() != null){
                    holder.tvChannel.setText(mySpaceList.get(position).getChannelInfo().getTitle());
                }*//*
            }
        }*/

        if(mySpaceList.get(position).getLikeInfo() != null){
            holder.tvLikeCount.setText(String.valueOf(mySpaceList.get(position).getLikeInfo().getLikeCount()));
        }
        ...
        int totalComment = mySpaceList.get(position).getCommentInfoCount()+mySpaceList.get(position).getReplyInfoCount();
        holder.tvCommentCount.setText(String.valueOf(totalComment));
    ...
    protected class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private AppCompatImageView ivPostImage , ivBookMark,tvUserVerifiedInfo,tvUserVerified , ivShare;
