...
public class HomeLatestAdapter extends PagedListAdapter<HomeLatestPostModel.MyLatestPost, HomeLatestAdapter.ViewHolder> {
    ...
    private int loginUserId ,darkModelStatus;
    private String followThread;
    MyUtility myUtility = new MyUtility();
    List<HomeLatestPostModel.MyLatestPost> listData;


    public HomeLatestAdapter(Context context , int showAnnoucement ,String followThread, CallLikeUnLike callLikeUnLike) {
        super(DEFF_CALLBACK);
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new HomeLatestAdapter.ViewHolder(view);
    }

    @SuppressLint({"ClickableViewAccessibility", "SetTextI18n"})
    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        HomeLatestPostModel.MyLatestPost latestPost = getItem(position);
        listData.add(latestPost);
       ...
       holder.tvPostTitle.setText(listData.get(position).getTitle());
    ...
    holder.tvGroup.setVisibility(View.GONE);
       ...
       // Log.e("new Comment=====> " , String.valueOf(latestPost.getTitle()));
      ...
      if(showAnnoucement == 0) { // ths condition use for show and hide addMySpaceView and Announcement
            ...
            if ((Utility.isNullOrEmpty(latestPost.getImg()) &&
                ...
                if (Utility.isNullOrEmpty(latestPost.getImg())) {
                    ...
                    if (latestPost.getColorCode().equalsIgnoreCase("#f1c40f")) {
                        ...
                        holder.tvCatchyName.setText(Html.fromHtml(latestPost.getCatchyText()));
                    } else {
                        holder.rlCathySection.setBackgroundColor(Color.parseColor(latestPost.getColorCode()));
        ...
        myUtility.homePageLikeUnLike(context,holder.ivLikeImage,holder.tvLikeCount,latestPost,loginUserId);
        ...
        holder.ivAddToMySpace.setOnClickListener(view -> {
            MyUtility m2 = new MyUtility();
            m2.addAddSpaceMusic(context);
            InternetConnection internetConnection = new InternetConnection();
            boolean onlineStatus  = internetConnection.checkConnection(context);
            if(onlineStatus){
                if(loginUserId != 0) {
                ...
                MyUtility myUtility = new MyUtility();
                myUtility.checkConnection(context);
            }
        ...
        holder.llMainPostSection.setOnClickListener(view -> {
            ...
            String image = sp.getData(BaseUrl.GALLERY_IMAGE);

            InternetConnection internetConnection = new InternetConnection();
            boolean onlineStatus  = internetConnection.checkConnection(context);
            if(onlineStatus){
                if(image != null){
                    Intent intent = new Intent(context , CreateCommentOnPostActivity.class);
      ...
      holder.llPostUserSection.setOnClickListener(view -> {
        ...
        holder.llLikeSection.setOnClickListener(v -> {
            MyUtility myUtility1 = new MyUtility();
            myUtility1.addLikeMusic(context);
            InternetConnection internetConnection = new InternetConnection();
            boolean onlineStatus  = internetConnection.checkConnection(context);
            if(onlineStatus){
                if(loginUserId == 0){
                    ...
                    if (latestPost.getLikeInfo() != null) {
                        callLikeUnLike.callMyLikeUnlike(holder.likeProgressBar, latestPost, position, latestPost.getLikeInfo().getLikeCount(), latestPost.getId(),
                                loginUserId, holder.ivLikeImage, holder.tvLikeCount ,holder.ivLiveHeart);
                    } else {
                        callLikeUnLike.callMyLikeUnlike(holder.likeProgressBar, latestPost, position, 0, latestPost.getId(),
                                loginUserId, holder.ivLikeImage, holder.tvLikeCount , holder.ivLiveHeart);
                    }
                }
            }else{
    ...
    private static DiffUtil.ItemCallback<HomeLatestPostModel.MyLatestPost> DEFF_CALLBACK =
            new DiffUtil.ItemCallback<HomeLatestPostModel.MyLatestPost>() {
                @Override
                public boolean areItemsTheSame(@NonNull HomeLatestPostModel.MyLatestPost oldItem, @NonNull HomeLatestPostModel.MyLatestPost newItem) {
                    return oldItem.getId() == newItem.getId();
                }

                @Override
                public boolean areContentsTheSame(@NonNull HomeLatestPostModel.MyLatestPost oldItem, @NonNull HomeLatestPostModel.MyLatestPost newItem) {
